...
# Data Structures
* [What is a priority queue](https://ru.wikipedia.org/wiki/Очередь_с_приоритетом_(программирование))? 
абстрактный тип данных в программировании, поддерживающий две обязательные операции — добавить элемент и извлечь максимум (минимум). Предполагается, что для каждого элемента можно вычислить его приоритет — действительное число или в общем случае элемент линейно упорядоченного множества. 
* [What is a heap](https://ru.wikipedia.org/wiki/Куча_(структура_данных))? 
...
* [What is B-tree](https://ru.wikipedia.org/wiki/B-дерево)? 
...
* [What is a 2-3-tree](https://ru.wikipedia.org/wiki/2-3-дерево)? 
структура данных, являющаяся B-деревом степени , каждый узел (страница) которого имеет либо два потомка и одно поле, либо три потомка и два поля. 
* [What is RB-tree? What is AVL-tree](https://ru.wikipedia.org/wiki/АВЛ-дерево)?
...
* [What is Set](https://ru.wikipedia.org/wiki/Множество_(тип_данных))?
...
HashMap структура данных, реализующая интерфейс ассоциативного массива, а именно, она позволяет хранить пары (ключ, значение) и выполнять три операции: операцию добавления новой пары, операцию поиска и операцию удаления пары по ключу.
HashSet структура данных, реализующая множество, позволяет хранить уникальные значение и выполнять три операции: операцию добавления, операцию поиска и операцию удаления значения.
